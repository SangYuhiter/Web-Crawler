...
def get_plan_info_pkuhsc():
    ...
    main_page_soup.prettify()
    year_and_district=main_page_soup.find_all(class_="link_new01")
    # 招生计划年份
    years = []
    for item in year_and_district[0].find_all(name='a'):
        years.append(item.string.strip())
    print(years)
    for year in years:
        # 构造链接
